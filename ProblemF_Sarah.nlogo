...
to go
  ...
  [
    let n 10
    let average 0
    let testChromosome [5 33 39 61 58];[5 33 39 61 58];[8 23 15 15 14]
...
to-report mutate [chrom]
  let final []
  let largest item (length chrom - 1) (sort chrom)
  let rest []
  ; how much to subtract
  let n random (largest + 1)
  foreach chrom [
    if ? != largest [set rest lput ? rest]
    ]
  let chosen item (random length rest) rest
  foreach chrom [
    ifelse ? = chosen [set final lput (? + n) final]
    [ifelse ? = largest [set final lput (? - n) final]
      [set final lput ? final]]
    ]
  report final
end
...
to setup-ports
  ...
  create-port 1[setxy 22 -19 set size 10] ;;greece/turkey (EM) ENTRY 9
